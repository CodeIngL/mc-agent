 ...
 * <p>Description: [功能描述：从异常抓取的缓存中,获取待发送的异常信息,然后通过http接口,向mc-server报告异常消息]</p>
 * <p>Company: </p>
 *
 * @author linj
 * @version 1.0
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                LOGGER.error("IOException occurred on sending exception info to MC server. exception info will be put back to cache and wait several seconds.! ", e);
                registry.putUpdate(exceptionHolder);
